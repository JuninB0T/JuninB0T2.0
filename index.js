...
const { modjunin } = require('./src/modmenu')
...
const { juninmenu } = require('./src/juninmenu')
...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5511965802190:+55 (11) 96580-2190\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["11965802190@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5511965802190@s.whatsapp.net'
                        /******ApiKey Input******/
...
case 'timer':
		...
		client.sendMessage(from, idioma(prefix, sender), text, {quoted: mek})
				break
				case 'juninmenu':
					client.sendMessage(from, juninmenu(prefix, sender), text, {quoted: mek})
							break
							case 'menuadmin':
               ...
               break
               case 'modmenu':
               client.sendMessage(from, modjunin(prefix, sender), text, {quoted: mek})
               break
				case 'demote':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedido recebido, adicionando o gay posição como ADM :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedido recebido, adicionando o gay na posição como ADM : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'snowwrite':
					...
					var gbl7 = gh.split("|")[0];
					var gbl8 = gh.split("|")[1];
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}snowwrite texto1|texto2, exemplo ${prefix}snowwrite Junin|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar pedidos ${prefix}marvellogo texto, por exemplo ${prefix}marvellogo Junin BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
               ...
               if (data.error) return reply(data.error)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual é o código de linguagem? ', text, {quoted: mek})
					...
					if (args.length < 2) return client.sendMessage(from, 'Cadê o texto seu gay', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'block':
					...
					client.sendMessage(from, `Pedidos recebidos, bloqueando ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					client.updatePresence(from, Presence.composing) 
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 Junin BOT 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 Junin BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'setppbot':
				client.updatePresence(from, Presence.composing) 
				if (!isQuotedImage) return reply(`Envie fotos com a legenda ${prefix}setbotpp ou tags de imagem que já foram enviadas`)
					if (!isOwner) return reply(mess.only.ownerB)
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`onde está o texto? exemplo ${prefix}logowolf Junin|BOT`)
					reply(mess.wait)
					...
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('digite 1 para ativar o menu hentai')
					if (Number(args[0]) === 1) {
						...
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply('❬ SUCESSO ❭ desativando o recurso de hentai neste grupo')
					} else {
						reply('digite 1 para ativar, 0 para desativar o modo hentai')
					}
					break	
				case 'bucin':
				...
				case 'logowolf2':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply(`Cadê o texto, hum\nExemplo: ${prefix}phlogo |Junin|BOT`)
					reply(mess.wait)
            ...
            break
					...
					case 'nsfwloli':
						try {
							...
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hmmmm loli ne boa punha amigo'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
					...
					case 'nsfwblowjob':
						try {
							...
							buffer = await getBuffer(res.result)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'N�o fa�a ingredientes para o tio comum'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
				...
				case 'nsfwass':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Tai a bunda da loli cuidado amigo fbi pd estar perto'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
					...
					case 'nsfwahegao':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ta ai man vai la boa punheta amigo'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
					...
					case 'nsfwarmpits':
				        ...
				        uptime = process.uptime()
                                ...
                                case 'neonlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}neonlogo JuninBOT`)
                                        reply(mess.wait)
                                ...
                                case 'neonlogo2':
                                        ...
                                        teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}neonlogo2 JuninBOT`)
                                        reply(mess.wait)
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`teksnya mana um\nContoh: ${prefix}lionlogo Junin|BOT`)
                                        reply(mess.wait)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                ...
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}burnpaper JuninBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'coffee':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}coffee JuninBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'lovepaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}lovepaper JuninBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodblock':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}woodblock JuninBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'qowheart':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}qowheart JuninBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'mutgrass':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}mutgrass JuninBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}undergocean JuninBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'woodenboards':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}woodenboards JuninBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
				...
				client.sendMessage(from, buffer, image, {caption: 'atumalakakkkk:)', quoted: mek})
                 break
                                case 'wolfmetal':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}wolfmetal JuninBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
                                ...
                                case 'metalictglow':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}metalictglow JuninBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Marque a pessoa que você deseja clonar a foto')
					...
					break
                                ...
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}pubglogo Junin|BOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                ...
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply(`onde está o texto hum\nExemplo: ${prefix}harrypotter JuninBOT`)
                                        if (!isUser) return reply(mess.only.daftarB)
